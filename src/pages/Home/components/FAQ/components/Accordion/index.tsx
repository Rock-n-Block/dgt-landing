...
export const AccordionItemComponent: FC<AccordionItemComponentProps> = ({
  ...
  return (
    <div className={cn(s.item_wrapper, wrapperClassName)}>
      <button
        className={cn(s.item_wrapper_head)}
        type="button"
        onClick={() => handleToggleItem(state)}
      >
        {title(state)}
      </button>
      <div className={cn(s.item_wrapper_body, { [s.active]: isOpen })}>{content(state)}</div>
    </div>
  );
};
